class KaomojiExtension {
  ...
  getAllData() {
    ...
    if (this.currentType === "kaomoji") {
      combined =
        typeof defaultKaomojiData !== "undefined"
          ? JSON.parse(JSON.stringify(defaultKaomojiData))
          : {};
      ...
      combined =
        typeof defaultSymbolsData !== "undefined"
          ? JSON.parse(JSON.stringify(defaultSymbolsData))
          : {};
    } else if (this.currentType === "emoji") {
      combined =
        typeof defaultEmojiData !== "undefined"
          ? JSON.parse(JSON.stringify(defaultEmojiData))
          : {};
    }
      ...
      try {
        ...
        if (confirm(`確定要匯入嗎？`)) {
          ...
          if (Array.isArray(data.history)) {
            ...
            const oldHistSym = new Set(
              this.history.kaomoji.map((h) => h.symbol)
            );
            data.history.forEach((h) => {
              if (!oldHistSym.has(h.symbol)) this.history.kaomoji.push(h);
            });
            this.history.kaomoji.sort(
              (a, b) => (b.timestamp || 0) - (a.timestamp || 0)
            );
            this.history.kaomoji = this.history.kaomoji.slice(
              0,
              this.maxHistorySize
            );
          ...
          if (data.custom) {
            Object.keys(data.custom).forEach((cat) => {
              if (!this.customKaomoji[cat])
                this.customKaomoji[cat] = data.custom[cat];
              else {
  ...
  renderKaomoji() {
      ...
      if (term.length > 0) {
        ...
        Object.entries(allData).forEach(([catName, catData]) => {
          ...
          const isCatMatch =
            ...
            catTags.some((t) => t && t.toLowerCase().includes(term));
    ...
    items.forEach((item) => {
      ...
      let badgeHtml = "";
      if (this.currentCategory !== "custom" && item.isCustom)
        badgeHtml = '<span class="badge badge-custom"></span>';
  ...
  async handleClick(symbol) {
    ...
    this.history[this.currentType] = currentHistory.slice(
      0,
      this.maxHistorySize
    );
    await this.saveData();
  ...
  openModal(mode, data = null) {
    ...
    if (mode === "edit" && data) {
      ...
      document.getElementById("kaomojiTags").value = data.tags
        ? data.tags.join(",")
        : "";
  ...
  async handleAdd(symbol, category, tagsStr) {
    ...
    if (!this.customKaomoji[category])
      this.customKaomoji[category] = { categoryTags: [category], items: [] };
  ...
  async handleDelete(symbol, category, autoSave = true) {
    if (this.customKaomoji[category]) {
      this.customKaomoji[category].items = this.customKaomoji[
        category
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener("click", async () => {
        try {
          ...
          let cat =
            catSelect.value === "new" ? newCatInput.value : catSelect.value;
    ...
    document.getElementById("deleteBtn").addEventListener("click", async () => {
      ...
      if (confirm(`確定要刪除 ${this.currentEditTarget.symbol} 嗎？`)) {
        await this.handleDelete(
          this.currentEditTarget.symbol,
          this.currentEditTarget.category
        );
        this.generateCategoryTabs();
        this.renderKaomoji();
        closeModal();
    ...
    document
      .getElementById("addKaomoji")
      .addEventListener("click", () => this.openModal("add"));

    const aboutModal = document.getElementById("aboutModal");
    document
      .getElementById("infoBtn")
      .addEventListener("click", () => (aboutModal.style.display = "block"));
    document
      .getElementById("closeAbout")
      .addEventListener("click", () => (aboutModal.style.display = "none"));
